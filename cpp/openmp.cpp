...
double denom2(int i, double t, double* knots, double alpha) {
    return 2 * sin(t - knots[i + 1]) * (cos(alpha) - 1);
}

double coeff1(int i, double t, double* knots, double alpha) {
    return (alpha == 0.0) ? 0.0 : (sin((t - knots[i]) / 2) * cos((alpha - t + knots[i]) / 2)) / sin(alpha / 2);
}

double top(int i, double t, double* knots, double alpha) {
    return sin(t - knots[i + 1] + alpha) - sin(t - knots[i + 1]) - sin(alpha);
}

double coeff2(int i, double t, double* knots, double alpha) {
    return (denom2(i, t, knots, alpha) == 0.0) ? 0.0 : top(i, t, knots, alpha) / denom2(i, t, knots, alpha);
}
...
double basisFunction(int i, int k, double t, double* knots, double alpha) {
    ...
    if (k == 2) {
        double p = 1 - coeff2(i + 1, t, knots, alpha);
        return coeff2(i, t, knots, alpha) * basisFunction(i, k - 1, t, knots, alpha) + p * basisFunction(i + 1, k - 1, t, knots, alpha);
    }
    double p = 1 - coeff1(i + 1, t, knots, alpha);
    return coeff1(i, t, knots, alpha) * basisFunction(i, k - 1, t, knots, alpha) + p * basisFunction(i + 1, k - 1, t, knots, alpha);
}
...
#pragma omp parallel for default(none) shared(controlPoints, numControlPoints, degree, knots, numPoints, curvePoints, alpha, numAxes, space)
...
int main(int argc, char* argv[]) {
    ...
    int numOfThreads = std::stoi(argv[8]);
